...
export const InvoiceCard: React.FC<InvoiceCardProps> = ({ invoice, onDelete, onStatusChange }) => {
    ...
    // Quick action: mark as paid (available for pending, sent, partial, overdue)
    const canMarkPaid = ['pending', 'sent', 'partial', 'overdue'].includes(invoice.invoiceStatus);

    // Swipe actions
    const deleteAction = onDelete ? createDeleteAction(() => onDelete(invoice.id)) : undefined;

    const quickAction = canMarkPaid && onStatusChange ? createQuickAction(
